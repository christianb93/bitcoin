...
def test_tc1():
    ...
    _payload = btc.keys.wifToPayloadBytes(wif)
    ...
    assert(wif == btc.keys.payloadValueToWif(d, version=239))
...
def test_tc2():
    ...
    _X, _Y = btc.keys.publicPointFromPEM(pem)
...
def test_tc3():
    ...
    X, Y = btc.keys.ecPointHexToValues(hex_uncompressed)
    assert(X == 29653386957644285094214794487095156456314684607297660684351363437249767222881)
    assert(Y == 86137829868349875656951651687933598437024590075828608787499242182788203988742)
    hex_compressed = "02418f38e4eb8a96a25020739c0da0bfd843cdbc311ccda4b1a688f3541c212e61"
    assert(btc.keys.ecPointCompress(X, Y) == bytes.fromhex(hex_compressed))
    assert(btc.keys.ecPointCompressHex(X, Y) == hex_compressed)
    
def test_tc4():
    hex_compressed = "02418f38e4eb8a96a25020739c0da0bfd843cdbc311ccda4b1a688f3541c212e61"
    assert("mitu3NFAd83mPQnVVu6k1yd47VWLJ9JATd" == btc.keys.ecAddress(hex_compressed, version=239))
    
# obtain public key hash160 from the address
def test_tc5():
    hex_compressed = "02418f38e4eb8a96a25020739c0da0bfd843cdbc311ccda4b1a688f3541c212e61"
    address = btc.keys.ecAddress(hex_compressed, version=239)
    h = btc.keys.ecAddressToPKH(address)
...
def test_tc6():
    ...
    assert("mx5zVKcjohqsu4G8KJ83esVxN52XiMvGTY" == btc.keys.ecAddress(hex_compressed, version=239))
